...
class {ProducerClassName}(BaseRobot):
    ...
    output_streams: list[str] = ["{PRIMARY_STREAM}"] # 🔧 自定义点: 修改输出 stream
    ...
    async def on_tick(self) -> None:
        ...
        await self.emit("{PRIMARY_STREAM}", "data_update", data)
        logger.debug("{ProducerClassName} 已发送数据信号: task={}", self.task_id)
