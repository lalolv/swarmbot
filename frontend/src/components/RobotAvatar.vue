...
const STATE_DEFAULT = { dotColor: "#ffee00", glowRadius: 1.5, bodyOpacity: 0.65, glow: false };

const info = computed(() => STATE_MAP[props.state] || STATE_DEFAULT);

const isRunning = computed(() => props.state === "running");

const filterId = computed(() => `glow-${props.uid}`);

const svgWidth = computed(() => 40 * props.size);
const svgHeight = computed(() => 44 * props.size);
</script>

<template>
  <div class="robot-avatar"
       :class="{ 'avatar-glow': info.glow }"
       :style="{ '--avatar-color': color, width: svgWidth + 'px', height: svgHeight + 'px' }">
...
.robot-avatar::after {
  ...
  inset: -2px;
  border-radius: 10px;
  background: radial-gradient(circle, color-mix(in srgb, var(--avatar-color) 15%, transparent) 0%, transparent 70%);
  pointer-events: none;
